...
def chatbot_response(user_input, user_id="default"):
    ...
    # Check if the user mentioned any of the supported websites
    match = re.search(r'.*?\b(go|open|visit)\b\s*(\w+)', user_input, re.IGNORECASE)
    if match:
        command = match.group(1).lower()  # Command (go, open, visit)
        website_name = match.group(2).lower()  # Website name (e.g., google, youtube)
        ...
        if website_name in supported_websites:
            ...
            return f"Here is {website_name}, and {website_name} is now open!", mode
        else:
            return f"Sorry, I don't recognize the website {website_name}. Please try again with a supported website like Google, YouTube.", mode


    if re.search(r'.*?\b(?:open|visit|launch|go|access|navigate\s*to)?\s*(lms\.pathein\.edu\.mm)\b',
    user_input, 
    re.IGNORECASE):
    ...
    match_google = re.search(
        r'\b(?:search|find|look\s*up|google)\s+(?:for\s+)?(.+?)(?:\s*\b(?:on\s+google)?)?$',
        user_input, 
        re.IGNORECASE
    )

    if match_google:
        search_query = match_google.group(1).strip()  # Capture the entire query and strip whitespace

    # Case 2: "search [query]" (without 'on google')
    elif re.search(r'\b(?:search|find)\s+(.+)', user_input, re.IGNORECASE):
        search_query = re.search(r'\b(?:search|find)\s+(.+)', user_input, re.IGNORECASE).group(1).strip()
    ...
    if re.search(r'.*?\b(weather\s\'?s?\s*(today|current\s+weather|weeather\s+today))\b', user_input, re.IGNORECASE):
    ...
    if user_id in waiting_for_meeting_info and waiting_for_meeting_info[user_id]:
        ...
        return f"Got it! Meeting name is '{user_input}'. Now provide the date and time (e.g., 2/10/Ye ) 10:00 AM).", mode
    ...
    calc_patterns = [
        ...
        r'\s*(?:=|equals)?\s*\??\s*$'  # Added support for "=" or "equals"
    ]
